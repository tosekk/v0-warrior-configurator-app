...
import { resolveBaseModelUrl, resolveModelUrls } from "@/lib/products";
...
export default function Home() {
  ...
  const [pendingRace, setPendingRace] = useState<"human" | "goblin" | null>(
    null,
  );
  ...
  async function loadUserData(userId: string) {
    ...
    if (configData) {
      ...
      console.log(
        "[v0] Loaded saved configuration. Race locked to:",
        savedRace,
      );
      ...
      console.log(
        "[v0] No saved configuration found - race selection is available",
      );
    ...
    if (purchasesData) {
      ...
      const completeBundleId =
        currentRace === "human"
          ? "human-complete-bundle"
          : "goblin-complete-bundle";
      const ownsCompleteBundle = purchasedProductIds.includes(completeBundleId);

      console.log(
        "[v0] User owns complete bundle for",
        currentRace,
        ":",
        ownsCompleteBundle,
      );
        ...
        import("@/lib/products").then(({ getProductsByRace }) => {
          ...
          const raceSpecificPurchases = purchasedProductIds.filter((id) => {
            return id.startsWith(
              currentRace === "human" ? "human-" : "goblin-",
            );
          });
          ...
          console.log(
            ...
            currentRace,
            ":",
            [...raceSpecificPurchases, ...allRaceItems].length,
            "items",
          );
        });
      } else {
  ...
  async function handleSaveConfiguration() {
    ...
    if (!ownershipValidation.valid) {
      console.error(
        "[v0] Ownership validation failed:",
      ...
      toast({
        ...
        description:
          ...
          "You must purchase these items first.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!result.success) {
      ...
      toast({
        ...
        description:
          result.error ||
          "Failed to save your warrior configuration. Please try again.",
        variant: "destructive",
      });
    } else {
      ...
      console.log(
        "[v0] Configuration saved successfully via",
        result.operation,
      );
  ...
  function handlePurchaseThemedBundle() {
    ...
    const bundleId =
      race === "human" ? "human-knight-set" : "goblin-raider-set";
  ...
  function handlePurchaseCompleteBundle() {
    ...
    const bundleId =
      race === "human" ? "human-complete-bundle" : "goblin-complete-bundle";
    setSelectedProductId(bundleId);
    setCheckoutDialogOpen(true);
  }
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="mb-6 flex justify-center">
          <Card className="p-2 bg-slate-900/50 border-slate-800">
            <Tabs
              value={race}
              onValueChange={(v) => handleRaceChange(v as "human" | "goblin")}
            >
